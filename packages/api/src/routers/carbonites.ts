  ...
  getAll: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      return await db
        ...
        .orderBy(asc(carbonites.state), asc(carbonites.office), asc(carbonites.pod), asc(carbonites.name));
    }),

  getById: protectedProcedure
    .input(z.object({ id: z.string() }))
    .query(async ({ input }) => {
      const [row] = await db.select().from(carbonites).where(eq(carbonites.id, input.id));
