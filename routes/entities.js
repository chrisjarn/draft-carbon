...
export async function entitiesRoutes(req, res, url) {
  ...
  if (req.method === 'GET' && !id) {
    const { rows } = await db.query('SELECT * FROM entities ORDER BY state, biz');
  ...
  if (req.method === 'PATCH' && id) {
    ...
    const allowed = ['biz','tan','office_id','state','phone','address','email'];
    const fields = [];
    const vals   = [];
    let i = 1;
    ...
    if ('sl'       in d) { fields.push(`sl = $${i++}`);       vals.push(JSON.stringify(d.sl)); }
    if ('partners' in d) { fields.push(`partners = $${i++}`); vals.push(JSON.stringify(d.partners)); }
...
function parse(row) {
  return {
    ...
    partners: Array.isArray(row.partners) ? row.partners : tryParse(row.partners, []),
  };
}
