	...
	return (
		<li className="py-2.5">
			...
			>
				...
				<p className="text-gray-900 text-sm dark:text-gray-50">
					{!read && (
						<span
							aria-hidden="true"
							className="mr-1.5 mb-px inline-flex size-2 shrink-0 rounded-full bg-blue-500 sm:text-sm dark:bg-blue-500"
						/>
					)}
					{message}
				</p>
				<p className="mt-2.5 text-gray-500 text-xs dark:text-gray-500">
					{formatDate(date)}
				</p>
			</a>
		</li>
		...
		<Popover>
			<PopoverTrigger asChild>
				<Button
					...
					className={cx(
						focusRing,
						"group rounded-full p-1 hover:bg-gray-100 data-[state=open]:bg-gray-100 data-[state=open]:dark:bg-gray-400/10 hover:dark:bg-gray-400/10",
					)}
				>
					<span className="flex size-8 items-center justify-center rounded-full border border-gray-300 bg-white p-1 dark:border-gray-700 dark:bg-gray-900 hover:dark:bg-gray-400/10">
						{unreadCount > 0 && (
							<span
								className="absolute top-2.5 right-2.5 size-2 shrink-0 rounded-full bg-blue-500"
								aria-hidden="true"
							/>
						)}
						<RiNotification2Line
