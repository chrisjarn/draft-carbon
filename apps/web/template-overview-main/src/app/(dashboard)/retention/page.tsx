	...
	return (
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContentFull className="fixed inset-4 mx-auto flex w-[95vw] flex-col overflow-hidden rounded-lg p-0 shadow-lg sm:max-w-3xl">
				<DialogHeader className="flex-none border-gray-200 border-b bg-zinc-50 px-6 py-4 dark:border-gray-900">
					...
					<DialogClose asChild>
						<Button className="absolute top-4 right-4 p-2" variant="ghost">
						...
						<section>
							...
							<div className="grid grid-cols-4 gap-4">
								{Object.entries(
									...
									>
										<span className="mb-1 block text-gray-600 text-sm capitalize dark:text-gray-400">
											{channel}
										</span>
										<span className="block font-medium text-lg">{value}%</span>
									</div>
								))}
							</div>
						</section>
				...
				<DialogFooter className="flex-none border-gray-200 border-t bg-white bg-zinc-50 px-6 py-4 dark:border-gray-900 dark:bg-[#090E1A]">
					<DialogClose asChild>
	...
	return (
		<main>
								...
								([cohortKey, cohortData]: [string, CohortData]) => (
									<TableRow key={cohortKey} className="h-full">
										<TableCell className="sticky left-0 z-10 h-full bg-white p-0 sm:min-w-56 dark:bg-gray-950">
											...
											>
												<RiExpandDiagonalLine className="absolute top-4 right-4 size-5 shrink-0 text-gray-500/0 transition group-hover:text-gray-500 group-focus-visible:text-gray-500" />
												<span className="block font-medium text-gray-900 text-sm dark:text-gray-50">
													{cohortKey}
												</span>
												<span className="mt-0.5 block text-gray-500 text-sm dark:text-gray-500">
										...
										{cohortData.weeks.map((weekData, weekIndex) => (
											...
											>
												{weekData === null ? (
													<div
														className={cx(
															"flex h-[64px] flex-col justify-center rounded border border-gray-200 border-dashed bg-gray-50 px-3.5 py-3 text-gray-200 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-800",
														)}
													>
			...
			<section className="mt-12">
				<h2 className="font-semibold text-gray-900 text-xl dark:text-gray-50">
					Cohort Analytics
				</h2>
				...
				<div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-8">
					<Card className="lg:col-span-6">
						...
						<dl className="mt-4 grid grid-cols-1 gap-6 md:grid-cols-3">
							...
							<div className="space-y-6">
								<div>
									<dt className="text-gray-500 text-sm dark:text-gray-500">
										Total Users
									</dt>
									<dd className="mt-1 flex items-baseline">
										<span className="font-semibold text-2xl text-gray-900 dark:text-gray-50">
											{cohortsAggregate.totalUsers.toLocaleString()}
										</span>
										<span className="ml-2 text-emerald-600 text-sm dark:text-emerald-500">
											+17%
										</span>
									</dd>
								</div>
									...
									return (
										...
										>
											<div className="flex items-center gap-2">
												...
												<span className="font-medium text-gray-900 text-sm dark:text-gray-50">
													{issue.category}
												</span>
											</div>
											<div className="text-gray-600 text-sm tabular-nums dark:text-gray-400">
