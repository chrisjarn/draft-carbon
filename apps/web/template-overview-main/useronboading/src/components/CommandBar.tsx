...
>(({ className, ...props }, ref) => {
	return (
		<div
			ref={ref}
			className={cx(
				"px-3 py-2.5 text-gray-300 text-sm tabular-nums",
				className,
			)}
			{...props}
		/>
...
>(({ className, ...props }, ref) => {
	return (
		<div
			ref={ref}
			className={cx(
				"relative flex items-center rounded-lg bg-gray-900 px-1 shadow-black/30 shadow-lg dark:ring-1 dark:ring-white/10",
				className,
			)}
			{...props}
		/>
...
const CommandBarCommand = React.forwardRef<HTMLButtonElement, CommandProps>(
	(
		...
		return (
			<span
				className={cx(
					"flex items-center gap-x-2 rounded-lg bg-gray-900 p-1 font-medium text-base text-gray-50 outline-none transition focus:z-10 sm:text-sm",
