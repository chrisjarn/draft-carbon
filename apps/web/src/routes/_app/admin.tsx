...
type ValidRole = "admin" | "practice_manager" | "sl_lead" | "state_manager" | "readonly";
...
function AdminPage() {
  ...
  const updateRole = useMutation(trpc.admin.updateRole.mutationOptions({
    onSuccess: (updated) => {
      qc.invalidateQueries({ queryKey: trpc.admin.listUsers.queryKey() });
      toast.success(`${updated.name} is now ${ROLES.find((r) => r.value === updated.role)?.label}`);
    },
                  ...
                  <TableRow key={u.id} className={isMe ? "bg-muted/20" : ""}>
                    ...
                    <TableCell>
                      {isMe ? (
                        ...
                        <Select
                          value={u.role}
                          onValueChange={(v) => updateRole.mutate({ userId: u.id, role: v as ValidRole })}
                    ...
                    <TableCell>
                      <Badge variant={u.emailVerified ? "outline" : "secondary"} className="text-[10px]">
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Delete <strong>{deleteTarget?.name}</strong> ({deleteTarget?.email})? This will remove their account and all sessions. This cannot be undone.
          </p>
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setDeleteTarget(null)}>Cancel</Button>
            <Button variant="destructive" size="sm" disabled={deleteUser.isPending}
