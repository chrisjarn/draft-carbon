	...
	const columns: ColumnDef<StaffWithMeta>[] = [
		...
		},
		{
			...
			cell: ({ getValue }) => {
				...
				return <span className="text-sm">{val ? slLabel(val) : "\u2014"}</span>;
			},
			meta: {
				label: "Service Line",
				variant: "multiSelect" as const,
		...
		},
		{
			...
			cell: ({ getValue }) => {
				const val = getValue() as string | null;
				return (
					<span className="text-sm">{val ? officeLabel(val) : "\u2014"}</span>
				);
			},
			meta: {
				label: "Office",
		...
		},
		{
			...
			cell: ({ row }) => {
				const manual = row.original.meta?.billingTarget;
				const hasManualOverride =
					manual != null && manual !== "" && Number(manual) > 0;
				...
				// Auto-calculate: salary * multiplier (simplified for FT doer)
				const salary = row.original.salary ?? 0;
				const autoCalc =
					salary > 0 ? Math.round(salary * DEFAULT_BILLING_MULT) : 0;
					...
					<TooltipProvider>
						<Tooltip>
							<TooltipTrigger
								...
								onClick={() => {
									if (opts.canEdit) {
										opts.onQuickUpsert(row.original.id, {
											billingTarget: String(autoCalc),
										});
									}
								}}
							>
							...
							<TooltipContent>
								Auto-calculated: {fmtDollar(autoCalc)} (salary {"\u00D7"}{" "}
								{DEFAULT_BILLING_MULT})
							</TooltipContent>
						</Tooltip>
					</TooltipProvider>
				);
