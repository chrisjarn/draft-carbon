...
export async function carbonitesRoutes(req, res, url) {
	...
	if (req.method === "GET" && id) {
		const { rows } = await db.query("SELECT * FROM carbonites WHERE id = $1", [
			id,
		]);
	...
	if (req.method === "PATCH" && id) {
		...
		if ("isPartner" in d) {
			fields.push(`is_partner = $${i++}`);
			vals.push(d.isPartner);
		}
		if ("reportsTo" in d) {
			fields.push(`reports_to = $${i++}`);
			vals.push(d.reportsTo);
		}
		if (!fields.length) return json(res, { error: "No fields to update" }, 400);
		vals.push(id);
		await db.query(
			`UPDATE carbonites SET ${fields.join(", ")} WHERE id = $${i}`,
			vals,
		);
		const { rows } = await db.query("SELECT * FROM carbonites WHERE id = $1", [
			id,
