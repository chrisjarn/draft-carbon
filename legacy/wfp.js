	...
	try {
		...
		if (p === "/api/wfp/staff-meta") {
			if (req.method === "GET") {
				...
				rows.forEach((r) => {
					out[r.cb_id] = {
						billingTarget: r.billing_target,
						perfRating: r.perf_rating,
						promoFlag: r.promo_flag,
						promoEta: r.promo_eta,
						staffRole: r.staff_role,
						billingActual: r.billing_actual,
					};
				});
				return json(res, out);
			}
          ...
          RETURNING *`,
					[
						d.cbId,
						d.billingTarget ?? null,
						d.perfRating ?? null,
						d.promoFlag ?? false,
						d.promoEta ?? null,
						d.staffRole ?? null,
						d.billingActual ?? null,
					],
				);
				return json(res, row);
			}
		...
		if (p === "/api/wfp/entity-settings") {
			if (req.method === "GET") {
				...
				const out = {};
				rows.forEach((r) => {
					out[r.ent_id] = { billingMultiplier: r.billing_multiplier, fy: r.fy };
				});
				return json(res, out);
			}
			if (req.method === "POST") {
		...
		if (p === "/api/wfp/revenue") {
			if (req.method === "GET") {
				...
				const out = {};
				rows.forEach((r) => {
					out[`${r.ent_id}|${r.fy}`] = { target: r.target, actual: r.actual };
				});
				return json(res, out);
			}
			if (req.method === "POST") {
